...
const TONE_STYLE: Record<Tone, string> = {
  ...
  muted: "text-[#1F2933] bg-[#667085]/10 border-[#D9DDE2]",
};
...
export function MetaBadge({
  ...
  return (
    ...
    >
      ...
      {hint && <HelpCircle size={11} className="text-[#667085]" />}
    </span>
  );
}
...
export function LevelTile({
  ...
  return (
    <div
      className={`rounded-[4px] border bg-[#F0F1EF] px-3 py-2.5 ${
    ...
    >
      ...
      <ul className="flex flex-col gap-1.5">
        {items.map((item) => (
          <li
            key={item}
            className="flex gap-2 text-[13px] leading-relaxed text-[#1F2933]"
          >
